        ...
        <div class="container">
                ...
                <form action="{{ route('kaprodi.kelas.plotmahasiswa', $kelas->id) }}" method="POST">
                    ...
                    <select name="mahasiswa_id" id="mahasiswa_id"
                        ...
                        @foreach ($availableMahasiswas as $mahasiswa)
                            <option value="{{ $mahasiswa->id }}">{{ $mahasiswa->name }} kelas asal
                                :{{ $mahasiswa->kelas->name }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="px-4 py-2 text-sm text-white rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark">Tambah
                        Mahasiswa</button>
                </form>
            </div>
            ...
            <div class="mt-4">
                ...
                <form action="{{ route('kaprodi.kelas.plotdosen', $kelas->id) }}" method="POST">
                    ...
                    <button type="submit" class="px-4 py-2 text-sm text-white rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring focus:ring-primary focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-dark">Tambah Dosen</button>
                </form>
            </div>
        </div>
</x-layout>
